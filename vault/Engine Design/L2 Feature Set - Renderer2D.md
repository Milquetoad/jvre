...
> 🚧 **Implementation underway (2026-06-13).** Beat 1: `Color` (sRGB→linear), the `shape2d` shaders (pixels→NDC, no Y-flip), and `Renderer2D`'s CPU half (`begin`/`fillRect`/`end`, unit-tested). **Beat 2 ✅ — first rectangles on screen**: `Pipeline` grew a third `Kind` (`SHAPES_2D`), a per-frame vertex arena (grown on overflow, uploaded like the UBO), a third content seam, `Renderer.renderer2D()`. Verified on the 4090 (solid + translucent fills, blending correct, validation silent). **Resize = shapes stay put in pixels** (the vertex shader divides by the live framebuffer size) — relative layout is user-side arithmetic against `g.width()/g.height()` (next), never a coordinate mode. **Beat 3**: the v1 **FILLS** ✅ (`fillRect`, `fillCircle`, `fillEllipse`, `fillTriangle`, `fillQuad`; + `g.width()/g.height()`) AND the v1 **STROKES** ✅ (`line` = a quad along the normal; `strokeRect` = a non-overlapping 8-triangle frame; `strokeCircle`/`strokeEllipse` = a tessellated ring; `strokeTriangle`/`strokeQuad` = `strokePolygon` with **miter joins**) — all CPU-triangulated, no `wideLines`, one shape batch. The **SDF render path** ✅ is in: `fillCircle` is now a signed-distance quad (analytic edge), sharing the one batch/pipeline with the flat shapes via a per-vertex coverage term — so **draw order is preserved** across flat and SDF shapes (the non-negotiable: every L2 primitive lives in one space, one paint order). The interface is unchanged; SDF only unlocks new primitives. **`fillRoundedRect`** ✅ followed -- via the rounded-box SDF, which *subsumes the circle* (`half=(r,r)`, `cornerRadius=r`), so both are one formula/two entry points. **`image`** ✅ (2026-06-14) -- the first gated content primitive: a textured quad on the SAME batch (vertex format widened to `[... | u v | mode]`, an explicit `mode` selector replacing the SDF sentinel; the shape pipeline grew its first descriptor set = one `COMBINED_IMAGE_SAMPLER`, re-pointed per frame, a 1x1 white default when no image is drawn). **Multi-texture batching** ✅ (2026-06-14) -- the one-texture limit is gone: the arena is sliced into **draw runs** (one `vkCmdDraw` per texture, paint order preserved), each run's set from a transient per-frame descriptor pool reset every frame (a single set can't be rebound between draws in one cmd buffer). Same-texture draws merge to one run (atlas = one draw). **`text`** ✅ (2026-06-14) -- the last gated primitive: **single-channel SDF glyphs** baked once (stb_truetype, built-in DejaVu Sans) into one R8 atlas, sampled as mode 3 (smoothstep over `fwidth` -> crisp at any size from one bake), one atlas = one run; `'\n'` newlines, top-left origin. **The L2 v1 surface is COMPLETE** (fills, strokes, SDF path, images, text). Catalogued post-v1 refinements: kerning, MSDF (sharp tiny-size corners), the combined `Style` fill+stroke, a custom-`Font` loading API, wider glyph coverage. See [[Progress Log]].
...
3. **Rotation belongs to the transform stack, not declaration modes.** `push() / translate(dx,dy) / rotate(angle) / scale(s) / pop()` — the one sanctioned "state", legal because it is explicitly scoped (push/pop pairs; `end()` asserts balance). This answers the *actual* demand behind Processing's `rectMode(CENTER)` (rotating about the centre). **✅ implemented 2026-06-14** (post-v1): JOML `Matrix3x2f` + a stack, applied CPU-side to vertex POSITIONS at append time (local/uv stay in-frame, interpolated like a UV); the SDF AA moved to a screen-space-gradient ramp so transformed SDF shapes stay crisp. Angles in radians. See [[Progress Log]].
...
| `fillCircle` / `strokeCircle(cx, cy, r, ...)` | centre + radius | SDF quad (mode 1 fill / mode 5 exact-annulus stroke) -- crisp at any size, MSAA-independent |
| `fillEllipse` / `strokeEllipse(cx, cy, rx, ry, ...)` | centre + radii | SDF quad (mode 4 fill / mode 5 ring) -- approximate ellipse distance (iq), exact for circles |
...
- **SDF edge-AA for L2 shapes ✅ (circle/rounded-rect from day one; ellipses + all curve strokes 2026-06-15)** -- circles/ellipses/strokes drawn as quads whose fragment shader ramps alpha over ~1px of signed distance to the edge. Better than MSAA for curves, costs nothing extra, independent of MSAA. The rounded box (mode 1) shipped with batching; `fillEllipse`/`strokeCircle`/`strokeEllipse` joined it (modes 4/5) once their tessellated edges were seen aliasing at MSAA 1 -- exact for circles, iq's approximation for ellipses.
