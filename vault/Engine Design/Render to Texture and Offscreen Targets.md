...
- **Per-pipeline UBO aliasing across passes.** A pipeline has one per-frame UBO. Draw it in *two* passes per frame with different uniforms and the second `uniform()` clobbers the first -- both draws render with the second matrix. That's why the demo routes the cube into a target *only* (drawn once). A sharp edge, not a wall -- it only bites the "same pipeline, twice/frame, different UBO" pattern, and it restricts nothing jvre can render. Workarounds, easiest first: (1) **push constants** for the per-draw-varying data (recorded INTO the command buffer per draw, so each draw keeps its own value -- a 64-byte mat4 MVP fits the 128-byte floor); (2) two pipeline instances (each its own UBO); (3) render once into a target and reuse the texture. The proper engine fix (catalogued): **dynamic-offset UBOs** (`UNIFORM_BUFFER_DYNAMIC` + a per-frame ring buffer, bound at a different offset per draw) -- makes one pipeline drawable N times/frame with N values, transparently.
