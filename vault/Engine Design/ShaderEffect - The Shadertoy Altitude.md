...
- **jvre's OWN built-in shaders** (the L2 `shape2d` vert+frag, the fullscreen-triangle vert) are fixed at build time -> the `compileShaders` Gradle task compiles them via the **bundled shaderc** (`jvre.tools.ShaderTool`, not the external `glslc`), shipping `.spv` resources. Zero runtime cost; errors break the build. (They live in `src/main/glsl/` so the task globs them automatically.) The demo's textured cube is NOT a built-in -- since the SCENE was retired it's authored in `Main` and compiled at RUNTIME through the public `ShaderCompiler`, like any user shader.
