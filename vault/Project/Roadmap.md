...
- **4d. Headless + frame readback. 🟡 READBACK DONE (2026-06-17); headless pending.** Render with no window (no swapchain) and copy the result image back to CPU memory. Beyond screenshots, the strategic payoff is **automated visual-regression testing** (render a frame -> diff a golden PNG) -- the one thing CI structurally can't do today.
  - **Readback ✅** -- `renderer.readPixels(target)` -> RGBA8 `byte[]` (image -> host-readable staging copy via `Commands.oneShot`; `Buffer.HostAccess.READ`/`readback`/`downloadBytes`; `Texture.renderTarget` gained `TRANSFER_SRC`; BGRA->RGBA swizzle). Demo: F2 writes a PNG of the L2 canvas (stb_image_write). On branch `feat/headless-readback` (parked -- folds into the format keystone below rather than shipping solo).
  - **Headless (pending)** -- a windowless `Device`+`Renderer` (no surface/swapchain/present). **Needs the format keystone** (Batch K below) -- the swapchain is today the source of color/depth/sample formats every pipeline bakes; headless has none. Needs a GPU/driver: Hal, a GPU CI runner, or a SOFTWARE Vulkan impl (lavapipe/SwiftShader) headless in CI. *Builds on 4a + the keystone.*
...
A batch of fully-fledged features requested 2026-06-17, parsed + grouped by subsystem, size (S/M/L), and -- the load-bearing part -- **shared prerequisites**, so a refactor isn't paid twice. Per [[jvre-fully-fledged-not-pull-based]] these are committed work. Sequence: **Batch 1 -> Batch 2 + 3-kerning -> the Keystone + headless + Batch 5 -> Batch 4 + the MSDF spike.**

- **Batch K -- the format keystone (the unlock).** Decouple color/depth/sample formats from the swapchain into a small "format source." Today the `Swapchain` IS the source of truth every pipeline + `RenderTarget` bakes. **Three things need this same refactor** -- do it once: **headless (4d)** (no swapchain -> no formats), **float/HDR targets** (Batch 5), and **per-target pipeline variants** (the catalogued 1x-target-on-MSAA case). Do the keystone before Batch 5 + headless.

- **Batch 1 -- OS / window surface (S each, independent, all GLFW, no render-core change).** Quick high-QoL wins; do first. Live in `Window`/`Input`:
  - Runtime window title (`glfwSetWindowTitle`); cursor shapes (`glfwCreateStandardCursor`/`glfwSetCursor`); clipboard get/set (`glfwGet/SetClipboardString`); file-drop callback (`glfwSetDropCallback` -> a per-frame dropped-paths list on the `Input` snapshot); HiDPI / content-scale clarity (`glfwGetWindowContentScale`; document framebuffer-px vs window-coords -- the L2 pixel space already = framebuffer px).

- **Batch 2 -- sampler / texture completeness** (the catalogued "sampler config"). Wrap/address modes (REPEAT / MIRRORED / CLAMP_TO_BORDER -- per-texture, S; today hardcoded CLAMP_TO_EDGE). Mipmaps (M -- generate the chain via `vkCmdBlitImage` + a mipmap sampler mode + per-mip layout transitions) + anisotropy (S, **but** needs a `samplerAnisotropy` **device-feature enable at `Device` creation** + the `maxSamplerAnisotropy` limit -- a small cross-layer coupling, see [[Game-Engine Capabilities (planned)]]).

- **Batch 3 -- text refinements.** **Kerning** (S, easy, independent -- `stbtt_GetCodepointKernAdvance`, adjust the pen in `text`/`textWidth`); do early. **MSDF** (**L + a decision** -- stb_truetype bakes only SINGLE-channel SDF; true MSDF needs an external generator (msdf-atlas-gen, offline) or hand-rolling, with no LWJGL binding -> likely "bundle pre-generated MSDF atlases," not runtime baking. Own spike before committing.) See [[l2-text-sdf-glyphs]].

- **Batch 4 -- shader authoring workflow.** Structured compile diagnostics from `ShaderCompiler` (S-M -- today it throws a raw error string; return parsed `(line, col, severity, message)` records / a typed exception). Pipeline/shader hot-rebuild hook (M -- rebuild a `Pipeline` from new SPIR-V; **also** the mechanism the swapchain-format-change rebuild already needs, so it pays double; enables ShaderEffect live-reload).

- **Batch 5 -- RTT / effect power** (on Batch K). Float/HDR targets (M -- `R16G16B16A16_SFLOAT`/etc.; needs the keystone + per-format pipeline variants). Bind N input textures as channels (M -- extend `PipelineSpec`/`ShaderEffect` to declare multiple `sampler2D` bindings, e.g. Shadertoy `iChannel0..3`; this is what makes post-processing *chains* real -- a pass samples the previous pass's target). Relates to the catalogued [[Game-Engine Capabilities (planned)|MRT/post-processing + HDR rows]].
