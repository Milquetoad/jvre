...
- **3a. L2 refinements.** **SDF curves (next)** -- move `fillEllipse`/`strokeCircle`/`strokeEllipse` from tessellation to the SDF path (crisp at any size, MSAA-independent): `strokeCircle` is an exact annulus SDF, the ellipses use an *approximate* ellipse distance (good enough for AA). The catalogued "SDF edge-AA for curves" track. Then: kerning + MSDF (text), the combined `Style` fill+stroke (the translucent-overlap *correctness* case), custom-font loading, sampler config (filter/mips/anisotropy), blend modes. Each ships when a consumer makes its absence felt.
