...
- **The sampler owns texel edges.** The checker boundaries are hard BECAUSE of NEAREST -- the pixel-art seam, kept deliberately. The fix-ladder, all sampler-side: **LINEAR mag filter (smooth up close) -- now SHIPPED** (the `Filter` enum, 2026-06-16; `loadImage` defaults to it); still future (catalogued in [[Game-Engine Capabilities (planned)]]): mipmaps + linear min (minification shimmer -- currently no mip chain, maxLod 0), anisotropic filtering (obliquely viewed surfaces -- the tumbling cube faces are literally the case the earlier anisotropy discussion said it pays off on).
