...
> **✅ BUILT as the bounded demo (2026-06-15).** Phase 3b shipped: `jvre.demo.Gui` (an immediate-mode label/button/slider toolkit over the **hot/active-ID** core) + `jvre.demo.GuiDemo` (a bouncing ball driven by the widgets; `gradlew runGuiDemo`). It is a **worked example built ON L2, deliberately NOT part of jvre's API** -- `jvre.demo` is excluded from the published jar, exactly like `jvre.tools`. Acting as a real L2 stress test it surfaced two missing public methods -- `Renderer2D.textWidth` / `lineHeight` (text measurement for centring/sizing) -- now added. The blueprint below (2026-06-09) is what it was built from; the cap held (label + button + slider, no checkbox/text-field/panel -- those stay blueprint-only unless a consumer needs them).
